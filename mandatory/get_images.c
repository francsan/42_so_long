...
/*   Updated: 2023/02/15 19:18:21 by francsan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_terrain(t_game *game)
{
	...
	if (!game->terrain)
	{
		...
		if (!game->terrain)
			error_msg(ERR_MEM);
		game->terrain[0] = mlx_xpm_file_to_image(game->mlx, \
		"imgs/0.xpm", &w, &h);
		game->terrain[1] = mlx_xpm_file_to_image(game->mlx, \
		"imgs/17.xpm", &w, &h);
		game->terrain[2] = mlx_xpm_file_to_image(game->mlx, \
		"imgs/18.xpm", &w, &h);
		game->terrain[3] = mlx_xpm_file_to_image(game->mlx, \
		"imgs/19.xpm", &w, &h);
	}
}

static void	get_fence_images(t_game *game)
{
	int	w;
	int	h;
